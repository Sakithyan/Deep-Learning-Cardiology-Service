...
from models.rnn import build_rnn_v1, build_rnn_v2, build_rnn_final
...
def evaluate_model(model, x_test, y_test):
    ...
    return {
        ...
        "inference_time_per_sample_ms": float((infer_s / len(x_test)) * 1000.0),
    }
...
def save_training_curves(history, model_type, output_dir, tag=""):
    ...
    path = os.path.join(output_dir, f"{model_type}{suffix}_training_curves.png")
...
def save_score_distribution(y_true, y_prob, model_type, output_dir, tag=""):
    ...
    fig.tight_layout()
    suffix = f"_{tag}" if tag else ""
    path = os.path.join(output_dir, f"{model_type}{suffix}_score_distribution.png")
    fig.savefig(path, dpi=150)
    plt.close(fig)
    return path
...
def save_mlp_final_diagnostic(y_prob, y_test, output_dir):
    ...
    axes[1].hist(y_prob[y_test == 0], bins=30, alpha=0.7, label="pas malade", color="tab:blue", edgecolor="black")
    axes[1].hist(y_prob[y_test == 1], bins=30, alpha=0.7, label="malade", color="tab:red", edgecolor="black")
    ...
    fig.tight_layout()
    path = os.path.join(output_dir, "mlp_final_diagnostic.png")
    fig.savefig(path, dpi=150)
    plt.close(fig)
    return path, roc_auc
...
def save_variant_comparison(history_a, history_b, model_type, output_dir):
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 4))

    ax1.plot(history_a.history["val_accuracy"], label="naive", color="tab:orange")
    ax1.plot(history_b.history["val_accuracy"], label="regularized", color="tab:green")
    ...
    ax2.plot(history_a.history["val_loss"], label="naive", color="tab:orange")
    ax2.plot(history_b.history["val_loss"], label="regularized", color="tab:green")
    ...
    fig.tight_layout()
    path = os.path.join(output_dir, f"{model_type}_variant_comparison.png")
    fig.savefig(path, dpi=150)
    plt.close(fig)
    return path
...
def run_repeats(model_type, variants, repeats=5, base_seed=42, epochs=35, batch_size=16, show_graphs=False):
    ...
    x_train, x_test, y_train, y_test = load_ecg_dataset(is_3d=is_3d_model(model_type))
...
def run_experiment(model_type="cnn", variant=None, compare_cnn=False, epochs=35, batch_size=16, show_graphs=False):
    ...
    graph_paths = [
        save_training_curves(history, model_type, output_dir, tag=selected_variant),
        save_confusion_matrix(y_test, result["y_pred"], model_type, output_dir, tag=selected_variant),
        save_score_distribution(y_test, result["y_prob"], model_type, output_dir, tag=selected_variant),
    ]
